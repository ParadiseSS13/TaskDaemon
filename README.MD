...
Pulls files from GitHub with a PAT to store in a docs database.

### Ingame Verified Sync

Tacks roles onto the relevant people for their ingame verified state
...
Adds/Removes authentik group for those who need Kibana access.
...
Pulls data from Patreon to store in a DB. This is then used for Discord role sync, and ingame perk granting.
...
- `TaskDaemon.Database.Authentik` - Database project automatically generated with JOOQ for the `authentik` database.
- `TaskDaemon.Database.Automation` - Database project automatically generated with JOOQ for the `aa07_automation` database.
